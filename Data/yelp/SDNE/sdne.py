...
class SDNE(object):
    def __init__(self, graph, nodes_u=[], nodes_v=[], hidden_size=[32, 16], alpha=1e-6, beta=5., nu1=1e-5, nu2=1e-4, ):
        ...
        self.A, self.L = self._create_A_L(
            self.graph, self.node2idx, nodes_u, nodes_v)  # Adj Matrix,L Matrix
        self.reset_model()
    ...
    def _get_context1(self, node_u, node_v, A):
        for index_i in tqdm(node_u):
            for index_j in node_u:
                weight = 0
                for index in node_v:
                    weight += A[index_i][index]
                if(index_i != index_j):
                    A[index_i][index_j]  += weight;
                    A[index_j][index_i]  += weight;
        for index_i in tqdm(node_v):
            for index_j in node_v:
                weight = 0
                for index in node_u:
                    weight += A[index_i][index]
                if(index_i != index_j):
    ...
    def _create_A_L(self, graph, node2idx, nodes_u=[], nodes_v=[]):
        ...
        if nodes_u != []:
            print("start")
            u = bipartite.weighted_projected_graph(graph, nodes_u, ratio=True)
            v = bipartite.weighted_projected_graph(graph, nodes_v, ratio=True)

            for edge in tqdm(u.edges()):
                v1, v2 = edge
                edge_weight = u[v1][v2].get('weight', 1)
                A[node2idx[v1]][node2idx[v2]] = edge_weight
                A[node2idx[v2]][node2idx[v1]] = edge_weight

            for edge in tqdm(v.edges()):
                v1, v2 = edge
                edge_weight = v[v1][v2].get('weight', 1)
                A[node2idx[v1]][node2idx[v2]] = edge_weight
                A[node2idx[v2]][node2idx[v1]] = edge_weight
            print(A[0].mean(), A[0].sum())
            print("end")
