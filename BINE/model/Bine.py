...
'''
    param:
        'train-data : Input graph file.')
        'model-name: name of model.')
        "vector-u: file of embedding vectors of U")
        "vector-v: file of embedding vectors of V")
        "case-train: file of training data for LR")
        "case-test: file of testing data for LR")
        'ws: window size.')
        'ns: number of negative samples.')
        'd: embedding size.')
        'maxT: maximal walks per vertex.')
        'minT: minimal walks per vertex.')
        'p: walk stopping probability.')
        'alpha = trade-off parameter alpha.')
        'beta: trade-off parameter beta.')
        'gamma: trade-off parameter gamma.')
        'lam : learning rate lambda.'
        'max-iter: maximal number of iterations.')
        'ton-n: recommend top-n items for each user.'
        'rec: calculate the recommendation metrics.')
        'lip: calculate the link prediction metrics.')
        'large : for large bipartite, 1 do not generate homogeneous graph file; 2 do not generate homogeneous graph'

'''


class BINE():
    def __init__(self,train-data=r'../data/rating_train.dat', test-data=r'../data/rating_test.dat', model-name='default', vectors-u=r'../data/vectors_u.dat',vectors-v=r'../data/vectors_v.dat',case-train=r'../data/wiki/case_train.dat', case-test=r'../data/wiki/case_test.dat', ws=5, ns=4
        d=128
        maxT=32
        minT=1
        p =0.15
